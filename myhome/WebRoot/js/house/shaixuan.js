...
//        var textTypeIndex =$(this).parents("dl").index();//鐖剁骇dl 鐨勭储寮曞�
//        var textType =$(this).parent("dd").siblings("dt").text();//鍙栦笂涓�眰鐨勬枃鏈�
//        index = textTypeIndex;
//        $(".clearDd").show();
...
var isCusPrice = false;//鏄惁鑷畾涔変环鏍�
var radioVal = '';
...
oSelectList.onclick = function(e, a) {
    ...
    if (tagName == 'INPUT') {
        if (tag.getAttribute('type').toUpperCase() == 'CHECKBOX') { //濡傛灉鐐瑰嚮 鐨勬槸 input checkbox
            var val = next(tag);
            if (tag.checked) {
        ...
        } else if (tag.getAttribute('type').toUpperCase() == 'BUTTON') { //濡傛灉鐐瑰嚮鐨勬槸 鑷畾涔変环鏍兼寜閽�
            radioVal = oCustext1.value + '-' + oCustext2.value + '鍏�;
            isCusPrice = true;
    ...
    if (tagName == 'A') { //濡傛灉鐐瑰嚮 鐨勬槸 A
        var oPrevInput = prev(tag);

        if (!oPrevInput) { //濡傛灉涓婁竴涓妭鐐规病鏈夊垯璁や负鐐瑰嚮鐨勬槸 '涓嶉檺'
            ...
            if (trim(prev(parent).innerHTML) == '閰掑簵浠锋牸') { //杩欓噷鏄洿鎺ユ牴鎹�text鏉ユ瘮杈冪殑.寤鸿鍔犱釜鑷畾涔夊睘鎬т綔鏍囪瘑绗�
                for (var i = 0; i < aRadio.length; i++) {
                    aRadio[i].checked = false;
                }
                isCusPrice = false;
            ...
            if (oPrevInput && oPrevInput.getAttribute('type').toUpperCase() == 'CHECKBOX') { //鑾峰彇checkbox
...
function delStr(str, arr) { //鍒犻櫎鏁扮粍缁欏畾鐩稿悓鐨勫瓧绗︿覆
    var n = -1;
    for (var i = 0,
...
function parents(elem, parents) {  //鏌ユ壘褰撳墠绁栧厛杈堝厓绱犻渶瑕佺殑鑺傜偣  濡�parents(oDiv, 'dd') 鏌ユ壘 oDiv 鐨勭鍏堝厓绱犱负dd 鐨�
